...
;              [libx.core :refer [store state] :as core]
;              [libx.spec.sub :as sub]
;              [libx.listeners :as l]
;              [libx.schema-fixture :refer [test-schema]]
;              [libx.util :refer [guid clara-tups->tups insert retract qa- entityv]]
;              [libx.tuplerules :refer [def-tuple-rule def-tuple-query def-tuple-session]]))
;
...
;  (cr/insert! [(guid) ::sub/response (libx.util/tuples->maps ?todos)]))
;
;(def-tuple-query find-all-facts
;  []
;  [?facts <- (acc/all) :from [:all]])
;
;(def-tuple-session my-session 'libx.store-test)
;
;(defn reset-store []
;  (reset! store {}))
;
