...
(defn all-facts [session]
  (mapv record->vec (:?facts (first (cr/query session find-all-facts)))))
...
(deftest user-schema-test
  ...
  (testing "Insert where user schema defines unique identity"
    (let [test-session @(def-tuple-session _test-session 'libx.schema-test 'libx.query)
