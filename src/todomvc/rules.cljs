(ns todomvc.rules
  ...
  (:require [clara.rules :refer [insert insert-all insert! insert-all! insert-all-unconditional!
...
;
(defquery really-find-todos
  []
  [?all <- (acc/all) :from [:todo/title]])

(defrule find-todos
  [?x <- :todo/status [[e a v]] (= v :done)]
  =>
  (println)
  (println "Found------------------------------ " ?x))
...
(defn todo [v]
  (let [temp-id (random-uuid)]
    [[temp-id :todo/title v]
     [temp-id :todo/status :done]]))

(todo "Hi")

(defsession todos 'todomvc.rules :fact-type-fn (fn [[e a v]]
                                                 (println [e a v])
                                                 a))

(def facts (reduce concat (repeatedly 5 #(todo "TODO"))))

(println "facts" facts)

(fire-rules (insert-all todos facts))

(query session really-find-todos)
