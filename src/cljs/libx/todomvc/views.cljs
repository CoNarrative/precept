...
(defn todo-item []
  (fn [{:keys [db/id todo/title todo/edit todo/dragging todo/done]}]
    (println "Todo item render: title, edit, done, dragging" title edit done dragging)
    [:li {:class (str (when done "completed " (when edit "editing"))) :id id}
      [:div.view
        [:input.toggle
          {:type "checkbox"
